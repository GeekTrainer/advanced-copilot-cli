# Section 5 — Adding a new feature: barcode support

| [← Previous: Shaping Copilot CLI's lifecycle with hooks][previous-lesson] | [Next: Modernizing apps with Copilot CLI →][next-lesson] |
|:--|--:|

Greenfield features in a brownfield app are where Copilot CLI earns its keep — *if* the work is scoped, researched, planned, and parallelized correctly. This section uses barcode / QR support for AssetTrack as the forcing function for `/research`, `/plan`, rubber-duck critique, custom agents (QA + accessibility), and `/fleet`.
...
> [!NOTE]
> **Starting state**: hooks from [Section 4][s04] in place; instructions, custom agents, and tests from [Sections 2–3][s02] in place. Exercises **modify code across one or more stacks** of AssetTrack under a feature branch (e.g., `feat/barcode-support`).
...
Next, you'll plan and execute a **modernization** of AssetTrack using `/research`, `/lsp`, MCP servers, and custom agents in [Section 6][next-lesson].
